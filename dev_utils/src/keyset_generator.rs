pub trait KeyGenerator {
    fn generate_keyset(&self, amount: usize) -> Vec<(Vec<u8>, Vec<u8>)>;
}
...
impl KeyGenerator for DenseKeyset {
    fn generate_keyset(&self, amount: usize) -> Vec<(Vec<u8>, Vec<u8>)> {
        let dense_fields = amount / self.length;
        ...
        let mut counter = 0;
        generate_keys(length, &mut buffer, &words, self.length, &mut counter, &mut ret);
...
impl KeyGenerator for ScrambledDenseKeyset {
    fn generate_keyset(&self, amount: usize) -> Vec<(Vec<u8>, Vec<u8>)> {
        let mut ret = DenseKeyset { length: 10000 }.generate_keyset(amount);
...
impl KeyGenerator for BadHeadsPercentage {
    fn generate_keyset(&self, amount: usize) -> Vec<(Vec<u8>, Vec<u8>)> {
        assert!(amount >= 5000);
        ...
        for prefix in 0..u32::MAX {
            for byte in 0..u8::MAX {
                for i in 0..20 {
                    ...
                    if i > self.percentage as usize / 5 {
...
#[cfg(test)]
mod test {
    use crate::keyset_generator::{BadHeadsPercentage, KeyGenerator};

    // checks probable heads position (key[4..8] is seen as heads)
    fn check_collision_percentage(kg: &dyn KeyGenerator, amount: usize) -> f32 {
    ...
    fn print_bad_heads_collisions() {
        let res = check_collision_percentage(&BadHeadsPercentage { percentage: 0 }, 5000);
        assert!(res < 0.01);
        let res = check_collision_percentage(&BadHeadsPercentage { percentage: 10 }, 5000);
        assert!(res < 0.11 && res > 0.09);
        let res = check_collision_percentage(&BadHeadsPercentage { percentage: 20 }, 5000);
        assert!(res < 0.21 && res > 0.19);
        let res = check_collision_percentage(&BadHeadsPercentage { percentage: 40 }, 5000);
        assert!(res < 0.41 && res > 0.39);
        let res = check_collision_percentage(&BadHeadsPercentage { percentage: 60 }, 5000);
        assert!(res < 0.61 && res > 0.59);
        let res = check_collision_percentage(&BadHeadsPercentage { percentage: 80 }, 5000);
