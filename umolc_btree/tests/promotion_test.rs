use dev_utils::keyset_generator::{BadHeadsKeyset, GoodHeadsKeyset, KeyGenerator, ScrambledDenseKeyset};
    ...
    let insert_key = |prefix: &[u8], i: u32, insert: bool| {
        ...
        if insert {
            ...
            let index_bytes: [u8; 4] =
                key[(b"Test").len()..].try_into().expect("Key does not contain valid u32 suffix");
    ...
    for iteration in 0..iterations {
        for i in 0..amount_keys {
            ...
            if i % (amount_keys / 4) == 0 {
                tree.scan(key.as_slice(), |_, _| false);
            }
        }
        ...
        for _ in 0..100 {
            tree.scan(first_key.as_slice(), |_, _| false);
        }
